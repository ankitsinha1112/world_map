import React from "react";
import "./Home.css";

export const Home = () => {
  return (
    <div>
      <div className="main">
        <div className="head">
          <h1>Countries and Economies</h1>
        </div>
        <hr />
        <div className="items">
          <div className="alpha">
            <a href="">A</a>
            <a href="">B</a>
            <a href="">C</a>
            <a href="">D</a>
            <a href="">E</a>
            <a href="">F</a>
            <a href="">G</a>
            <a href="">H</a>
            <a href="">I</a>
            <a href="">J</a>
            <a href="">K</a>
            <a href="">L</a>
            <a href="">M</a>
            <a href="">N</a>
            <a href="">O</a>
            <a href="">P</a>
            <a href="">Q</a>
            <a href="">R</a>
            <a href="">S</a>
            <a href="">T</a>
            <a href="">U</a>
            <a href="">V</a>
            <a href="">W</a>
            <a href="">X</a>
            <a href="">Y</a>
            <a href="">Z</a>
          </div>
          <div className="search">
            <input type="text" />
          </div>
        </div>
        <hr />
        <div className="countries">
          <h2>A</h2>
          <div className="sec">
            <div className="sec1">
              <li>
                <a href="">Afghanistan</a>
              </li>
              <li>
                <a href="">Algeria</a>
              </li>
              <li>
                <a href="">Andorra</a>
              </li>
              <li>
                <a href="">Antigua and Barbuda</a>
              </li>
              <li>
                <a href="">Armenia</a>
              </li>
              <li>
                <a href="">Australia</a>
              </li>
              <li>
                <a href="">Azerbaijan</a>
              </li>
            </div>
            <div className="sec2">
              <li>
                <a href="">Albania</a>
              </li>
              <li>
                <a href="">American Samoa</a>
              </li>
              <li>
                <a href="">Angola</a>
              </li>
              <li>
                <a href="">Argentina</a>
              </li>
              <li>
                <a href="">Aruba</a>
              </li>
              <li>
                <a href="">Austria</a>
              </li>
            </div>
          </div>
        </div>
        <hr />
        <div className="countries">
          <h2>B</h2>
          <div className="sec">
            <div className="sec1">
              <li>
                <a href="">Bahamas, The</a>
              </li>
              <li>
                <a href="">Bangladesh</a>
              </li>
              <li>
                <a href="">Belarus</a>
              </li>
              <li>
                <a href="">Belize</a>
              </li>
              <li>
                <a href="">Bermuda</a>
              </li>
              <li>
                <a href="">Bolivia</a>
              </li>
              <li>
                <a href="">Botswana</a>
              </li>
              <li>
                <a href="">British Virgin Islands</a>
              </li>
              <li>
                <a href="">Bulgaria</a>
              </li>
              <li>
                <a href="">Burundi</a>
              </li>
            </div>
            <div className="sec2">
              <li>
                <a href="">Bahrain</a>
              </li>
              <li>
                <a href="">Barbados</a>
              </li>
              <li>
                <a href="">Belgium</a>
              </li>
              <li>
                <a href="">Benin</a>
              </li>
              <li>
                <a href="">Bhutan</a>
              </li>
              <li>
                <a href="">Bosnia and Herzegovina</a>
              </li>
              <li>
                <a href="">Brazil</a>
              </li>
              <li>
                <a href="">Brunei Darussalam</a>
              </li>
              <li>
                <a href="">Burkina Faso</a>
              </li>
            </div>
          </div>
        </div>
        <hr />
        <div className="countries">
          <h2>C</h2>
          <div className="sec">
            <div className="sec1">
              <li>
                <a href="">Cabo Verde</a>
              </li>
              <li>
                <a href="">Cameroon</a>
              </li>
              <li>
                <a href="">Cayman Islands</a>
              </li>
              <li>
                <a href="">Chad</a>
              </li>
              <li>
                <a href="">Chile</a>
              </li>
              <li>
                <a href="">Colombia</a>
              </li>
              <li>
                <a href="">Congo, Dem. Rep.</a>
              </li>
              <li>
                <a href="">Costa Rica</a>
              </li>
              <li>
                <a href="">Croatia</a>
              </li>
              <li>
                <a href="">Curacao</a>
              </li>
              <li>
                <a href="">Czech Republic</a>
              </li>
            </div>
            <div className="sec2">
              <li>
                <a href="">Cambodia</a>
              </li>
              <li>
                <a href="">Canada</a>
              </li>
              <li>
                <a href="">Central African Republic</a>
              </li>
              <li>
                <a href="">Channel Islands</a>
              </li>
              <li>
                <a href="">China</a>
              </li>
              <li>
                <a href="">Comoros</a>
              </li>
              <li>
                <a href="">Congo, Rep.</a>
              </li>
              <li>
                <a href="">Cote d'Ivoire</a>
              </li>
              <li>
                <a href="">Cuba</a>
              </li>
              <li>
                <a href="">Cyprus</a>
              </li>
            </div>
          </div>
        </div>
        <hr />
        <div className="countries">
          <h2>D</h2>
          <div className="sec">
            <div className="sec1">
              <li>
                <a href="">Denmark</a>
              </li>
              <li>
                <a href="">Dominica</a>
              </li>
            </div>
            <div className="sec2">
              <li>
                <a href="">Djibouti</a>
              </li>
              <li>
                <a href="">Dominican Republic</a>
              </li>
            </div>
          </div>
        </div>
        <hr />
        <div className="countries">
          <h2>E</h2>
          <div className="sec">
            <div className="sec1">
              <li>
                <a href="">Ecuador</a>
              </li>
              <li>
                <a href="">El Salvador</a>
              </li>
              <li>
                <a href="">Eritrea</a>
              </li>
              <li>
                <a href="">Eswatini</a>
              </li>
            </div>
            <div className="sec2">
              <li>
                <a href="">Egypt, Arab Rep.</a>
              </li>
              <li>
                <a href="">Equatorial Guinea</a>
              </li>
              <li>
                <a href="">Estonia</a>
              </li>
              <li>
                <a href="">Ethiopia</a>
              </li>
            </div>
          </div>
        </div>
        <hr />
        <div className="countries">
          <h2>F</h2>
          <div className="sec">
            <div className="sec1">
              <li>
                <a href="">Faroe Islands</a>
              </li>
              <li>
                <a href="">Finland</a>
              </li>
              <li>
                <a href="">French Polynesia</a>
              </li>
              
            </div>
            <div className="sec2">
              <li>
                <a href="">Fiji</a>
              </li>
              <li>
                <a href="">France</a>
              </li>
             
            </div>
          </div>
        </div>
        <hr />
        <div className="countries">
          <h2>G</h2>
          <div className="sec">
            <div className="sec1">
              <li>
                <a href="">Gabon</a>
              </li>
              <li>
                <a href="">Georgia</a>
              </li>
              <li>
                <a href="">Ghana</a>
              </li>
              <li>
                <a href="">Greece</a>
              </li>
              <li>
                <a href="">Grenada</a>
              </li>
              <li>
                <a href="">Guatemala</a>
              </li>
              <li>
                <a href="">Guinea-Bissau</a>
              </li>
            </div>
            <div className="sec2">
              <li>
                <a href="">Gambia, The</a>
              </li>
              <li>
                <a href="">Germany</a>
              </li>
              <li>
                <a href="">Gibraltar</a>
              </li>
              <li>
                <a href="">Greenland</a>
              </li>
              <li>
                <a href="">Guam</a>
              </li>
              <li>
                <a href="">Guinea</a>
              </li>
              <li>
                <a href="">Guyana</a>
              </li>
            </div>
          </div>
        </div>
        <hr />
        <div className="countries">
          <h2>H</h2>
          <div className="sec">
            <div className="sec1">
              <li>
                <a href="">Haiti</a>
              </li>
              <li>
                <a href="">Hong Kong SAR, China</a>
              </li>
            </div>
            <div className="sec2">
              <li>
                <a href="">Honduras</a>
              </li>
              <li>
                <a href="">Hungary</a>
              </li>
            </div>
          </div>
        </div>
        <hr />
        <div className="countries">
          <h2>I</h2>
          <div className="sec">
            <div className="sec1">
              <li>
                <a href="">Iceland</a>
              </li>
              <li>
                <a href="">Indonesia</a>
              </li>
              <li>
                <a href="">Iraq</a>
              </li>
              <li>
                <a href="">Isle of Man</a>
              </li>
              <li>
                <a href="">Italy</a>
              </li>
            </div>
            <div className="sec2">
              <li>
                <a href="">India</a>
              </li>
              <li>
                <a href="">Iran, Islamic Rep.</a>
              </li>
              <li>
                <a href="">Ireland</a>
              </li>
              <li>
                <a href="">Israel</a>
              </li>
            </div>
          </div>
        </div>
        <hr />
        <div className="countries">
          <h2>J</h2>
          <div className="sec">
            <div className="sec1">
              <li>
                <a href="">Jamaica</a>
              </li>
              <li>
                <a href="">Jordan</a>
              </li>
            </div>
            <div className="sec2">
              <li>
                <a href="">Japan</a>
              </li>
             
            </div>
          </div>
        </div>
        <hr />
        <div className="countries">
          <h2>K</h2>
          <div className="sec">
            <div className="sec1">
              <li>
                <a href="">Kazakhstan</a>
              </li>
              <li>
                <a href="">Kiribati</a>
              </li>
              <li>
                <a href="">Korea, Rep.</a>
              </li>
              <li>
                <a href="">Kuwait</a>
              </li>
            </div>
            <div className="sec2">
              <li>
                <a href="">Kenya</a>
              </li>
              <li>
                <a href="">Korea, Dem. People's Rep.</a>
              </li>
              <li>
                <a href="">Kosovo</a>
              </li>
              <li>
                <a href="">Kyrgyz Republic</a>
              </li>
            </div>
          </div>
        </div>
        <hr />
        <div className="countries">
          <h2>L</h2>
          <div className="sec">
            <div className="sec1">
              <li>
                <a href="">Lao PDR</a>
              </li>
              <li>
                <a href="">Lebanon</a>
              </li>
              <li>
                <a href="">Liberia</a>
              </li>
              <li>
                <a href="">Liechtenstein</a>
              </li>
              <li>
                <a href="">Luxembourg</a>
              </li>
            </div>
            <div className="sec2">
              <li>
                <a href="">Latvia</a>
              </li>
              <li>
                <a href="">Lesotho</a>
              </li>
              <li>
                <a href="">Libya</a>
              </li>
              <li>
                <a href="">Lithuania</a>
              </li>
            </div>
          </div>
        </div>
        <hr />
        <div className="countries">
          <h2>M</h2>
          <div className="sec">
            <div className="sec1">
              <li>
                <a href="">Macao SAR, China</a>
              </li>
              <li>
                <a href="">Malawi</a>
              </li>
              <li>
                <a href="">Maldives</a>
              </li>
              <li>
                <a href="">Malta</a>
              </li>
              <li>
                <a href="">Mauritania</a>
              </li>
              <li>
                <a href="">Mexico</a>
              </li>
              <li>
                <a href="">Moldova</a>
              </li>
              <li>
                <a href="">Mongolia</a>
              </li>
              <li>
                <a href="">Morocco</a>
              </li>
              <li>
                <a href="">Myanmar</a>
              </li>
              
            </div>
            <div className="sec2">
              <li>
                <a href="">Madagascar</a>
              </li>
              <li>
                <a href="">Malaysia</a>
              </li>
              <li>
                <a href="">Mali</a>
              </li>
              <li>
                <a href="">Marshall Islands</a>
              </li>
              <li>
                <a href="">Mauritius</a>
              </li>
              <li>
                <a href="">Micronesia, Fed. Sts.</a>
              </li>
              <li>
                <a href="">Monaco</a>
              </li>
              <li>
                <a href="">Montenegro</a>
              </li>
              <li>
                <a href="">Mozambique</a>
              </li>
             
            </div>
          </div>
        </div>
        <hr />
        <div className="countries">
          <h2>N</h2>
          <div className="sec">
            <div className="sec1">
              <li>
                <a href="">Namibia</a>
              </li>
              <li>
                <a href="">Nepal</a>
              </li>
              <li>
                <a href="">New Caledonia</a>
              </li>
              <li>
                <a href="">Nicaragua</a>
              </li>
              <li>
                <a href="">Nigeria</a>
              </li>
              <li>
                <a href="">Northern Mariana Islands</a>
              </li>
            </div>
            <div className="sec2">
              <li>
                <a href="">Nauru</a>
              </li>
              <li>
                <a href="">Netherlands</a>
              </li>
              <li>
                <a href="">New Zealand</a>
              </li>
              <li>
                <a href="">Niger</a>
              </li>
              <li>
                <a href="">North Macedonia</a>
              </li>
              <li>
                <a href="">Norway</a>
              </li>
            </div>
          </div>
        </div>
        <hr />
        <div className="countries">
          <h2>O</h2>
          <div className="sec">
            <div className="sec1">
              <li>
                <a href="">Oman</a>
              </li>
             
            </div>
            <div className="sec2">
            
             
            </div>
          </div>
        </div>
        <hr />
        <div className="countries">
          <h2>P</h2>
          <div className="sec">
            <div className="sec1">
              <li>
                <a href="">Pakistan</a>
              </li>
              <li>
                <a href="">Panama</a>
              </li>
              <li>
                <a href="">Paraguay</a>
              </li>
              <li>
                <a href="">Philippines</a>
              </li>
              <li>
                <a href="">Portugal</a>
              </li>
            </div>
            <div className="sec2">
              <li>
                <a href="">Palau</a>
              </li>
              <li>
                <a href="">Papua New Guinea</a>
              </li>
              <li>
                <a href="">Peru</a>
              </li>
              <li>
                <a href="">Poland</a>
              </li>
              <li>
                <a href="">Puerto Rico</a>
              </li>
            </div>
          </div>
        </div>
        <hr />
        <div className="countries">
          <h2>Q</h2>
          <div className="sec">
            <div className="sec1">
              <li>
                <a href="">Qatar</a>
              </li>
             
            </div>
            <div className="sec2">
            
             
            </div>
          </div>
        </div>
        <hr />
        <div className="countries">
          <h2>R</h2>
          <div className="sec">
            <div className="sec1">
              <li>
                <a href="">Romania</a>
              </li>
              <li>
                <a href="">Rwanda</a>
              </li>
            </div>
            <div className="sec2">
              <li>
                <a href="">Russian Federation</a>
              </li>
             
            </div>
          </div>
        </div>
        <hr />
        <div className="countries">
          <h2>S</h2>
          <div className="sec">
            <div className="sec1">
              <li>
                <a href="">Samoa</a>
              </li>
              <li>
                <a href="">Sao Tome and Principe</a>
              </li>
              <li>
                <a href="">Senegal</a>
              </li>
              <li>
                <a href="">Seychelles</a>
              </li>
              <li>
                <a href="">Singapore</a>
              </li>
              <li>
                <a href="">Slovak Republic</a>
              </li>
              <li>
                <a href="">Solomon Islands</a>
              </li>
              <li>
                <a href="">South Africa</a>
              </li>
              <li>
                <a href="">Spain</a>
              </li>
              <li>
                <a href="">St. Kitts and Nevis</a>
              </li>
              <li>
                <a href="">St. Martin (French part)</a>
              </li>
              <li>
                <a href="">Sudan</a>
              </li>
              <li>
                <a href="">Sweden</a>
              </li>
              <li>
                <a href="">Syrian Arab Republic</a>
              </li>
            </div>
            <div className="sec2">
              <li>
                <a href="">San Marino</a>
              </li>
              <li>
                <a href="">Saudi Arabia</a>
              </li>
              <li>
                <a href="">Serbia</a>
              </li>
              <li>
                <a href="">Sierra Leone</a>
              </li>
              <li>
                <a href="">Sint Maarten (Dutch part)</a>
              </li>
              <li>
                <a href="">Slovenia</a>
              </li>
              <li>
                <a href="">Somalia</a>
              </li>
              <li>
                <a href="">South Sudan</a>
              </li>
              <li>
                <a href="">Sri Lanka</a>
              </li>
              <li>
                <a href="">St. Lucia</a>
              </li>
              <li>
                <a href="">St. Vincent and the Grenadines</a>
              </li>
              <li>
                <a href="">Suriname</a>
              </li>
              <li>
                <a href="">Switzerland</a>
              </li>
            </div>
          </div>
        </div>
        <hr />
        <div className="countries">
          <h2>T</h2>
          <div className="sec">
            <div className="sec1">
              <li>
                <a href="">Tajikistan</a>
              </li>
              <li>
                <a href="">Thailand</a>
              </li>
              <li>
                <a href="">Togo</a>
              </li>
              <li>
                <a href="">Trinidad and Tobago</a>
              </li>
              <li>
                <a href="">Turkey</a>
              </li>
              <li>
                <a href="">Turks and Caicos Islands</a>
              </li>
            </div>
            <div className="sec2">
              <li>
                <a href="">Tanzania</a>
              </li>
              <li>
                <a href="">Timor-Leste</a>
              </li>
              <li>
                <a href="">Tonga</a>
              </li>
              <li>
                <a href="">Tunisia</a>
              </li>
              <li>
                <a href="">Turkmenistan</a>
              </li>
              <li>
                <a href="">Tuvalu</a>
              </li>
            </div>
          </div>
        </div>
        <hr />
        <div className="countries">
          <h2>U</h2>
          <div className="sec">
            <div className="sec1">
              <li>
                <a href="">Uganda</a>
              </li>
              <li>
                <a href="">United Arab Emirates</a>
              </li>
              <li>
                <a href="">United States</a>
              </li>
              <li>
                <a href="">Uzbekistan</a>
              </li>
            </div>
            <div className="sec2">
              <li>
                <a href="">Ukraine</a>
              </li>
              <li>
                <a href="">United Kingdom</a>
              </li>
              <li>
                <a href="">Uruguay</a>
              </li>
            
            </div>
          </div>
        </div>
        <hr />
        <div className="countries">
          <h2>V</h2>
          <div className="sec">
            <div className="sec1">
              <li>
                <a href="">Vanuatu</a>
              </li>
              <li>
                <a href="">Vietnam</a>
              </li>
            </div>
            <div className="sec2">
              <li>
                <a href="">Venezuela, RB</a>
              </li>
              <li>
                <a href="">Virgin Islands (U.S.)</a>
              </li>
            </div>
          </div>
        </div>
        <hr />
        <div className="countries">
          <h2>W</h2>
          <div className="sec">
            <div className="sec1">
              <li>
                <a href="">West Bank and Gaza</a>
              </li>
             
            </div>
            <div className="sec2">
            
             
            </div>
          </div>
        </div>
        <hr />
        <div className="countries">
          <h2>Y</h2>
          <div className="sec">
            <div className="sec1">
              <li>
                <a href="">Yemen, Rep.</a>
              </li>
             
            </div>
            <div className="sec2">
            
             
            </div>
          </div>
        </div>
        <hr />
        <div className="countries">
          <h2>Z</h2>
          <div className="sec">
            <div className="sec1">
              <li>
                <a href="">Zambia</a>
              </li>
             
            </div>
            <div className="sec2">
            <li>
                <a href="">Zimbabwe</a>
              </li>
             
            </div>
          </div>
        </div>
        <hr />
      </div>
    </div>
  );
};
